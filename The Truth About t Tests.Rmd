...
1) Samples are taken from normally distributed populations.  If not, we may get around this with the Central Limit Theorem, if the size of the sample is ~25-30+.
...
6) Independent samples of size 10, given correlation of 0.7 within populations
7) Two samples from N(100, 10), correlation of 0.7 between populations

We will investigate if the significance level is truly equal to 0.05 in all of these cases.   Recall that the significance level is the probability of Type I error, i.e. P(reject Ho | Ho true).  We will use the two-sided test.
...
monteCarloT <- function(nrep, nsim, x, y, crit, pool){
  ...
  # Initial vectors for storing simulation results
  ...
  # Return Type I error rate and corresponding standard error
  p <- mean(prob)
...
### Independent samples of size 10, given correlation of 0.7 within populations ###
